...
public class RecyclerItemTouchHelperLeft extends ItemTouchHelper.SimpleCallback {
    ...
    public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
        if (viewHolder != null) {
            final View foregroundView = ((StaffRecyclerAdapter.StaffHolder) viewHolder).viewForeground;
            getDefaultUIUtil().onSelected(foregroundView);

            /* change background depend on action */
            ((StaffRecyclerAdapter.StaffHolder) viewHolder).viewBackgroundDelete.setVisibility(View.VISIBLE);
            ((StaffRecyclerAdapter.StaffHolder) viewHolder).viewBackgroundDelete.invalidate();
